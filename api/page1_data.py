...
def add_course(course: Course):
    with db.get_connection() as conn:
        conn.execute("INSERT INTO course (name) VALUES (?)", (course.name,))
        conn.commit()
...
def add_subject(subject: Subject):
    with db.get_connection() as conn:
        conn.execute("INSERT INTO subject (course_id, name) VALUES (?, ?)", (subject.course_id, subject.name))
        conn.commit()
...
def add_topic(topic: Topic):
    with db.get_connection() as conn:
        conn.execute("INSERT INTO topic (subject_id, name) VALUES (?, ?)", (topic.subject_id, topic.name))
        conn.commit()
